...
export const getVcaResult = (stock) => {
  ...
  return (
    <>
      ...
      <p>{`jumlah stock yang saya miliki sejak ${monthDifference} bulan terakhir adalah ${(
        stock?.totalLot * 100
      ).toLocaleString("id-ID")} Unit
        (${stock?.totalLot.toLocaleString("id-ID")} Lot)`}</p>
      <p>{`total investasi yang saya miliki sejak ${monthDifference} bulan terakhir adalah ${rp(
        stock?.totalPrice()
      )}`}</p>
