...
import { VcaCalculator } from "./VcaCalculator";
import { getVcaResult } from "./VcaFormResult";
...
const Vca = () => {
  ...
  return (
    <>
      <form id="survey-form" method="GET" action="#" onSubmit={calculate}>
        <div className="rowTab">
          <div className="labels">
            <label id="name-label" htmlFor="stockName">Nama Emiten: </label>
          </div>
          <div className="rightTab">
            <input type="text" name="stockName" id="stockName" className="input-field"
                   placeholder="BYAN" style={{ "textTransform": "uppercase" }} required={true}
            />
          </div>
        </div>
        <div className="rowTab">
          <div className="labels">
            <label id="email-label" htmlFor="sinceMonth">Mulai sejak: </label>
          </div>
          <div className="rightTab" key={"since"}>
            ...
            <input type="number" name="sinceYear" id="sinceYear" className="input-field"
                   placeholder="2022" style={{ width: "25%" }} min={1970} max={3000} required={true} />
          </div>
        </div>
        <div className="rowTab">
          <div className="labels">
            <label id="number-label" htmlFor="totalLot">Total stock yang dimiliki saat ini: </label>
          </div>
          <div className="rightTab" key={"totalLot"}>
            <input type="number" name="totalLot" id="totalLot" className="input-field"
                   style={{ width: "40%" }} min={0} placeholder="10" required={true} />
        ...
        <div className="rowTab">
          <div className="labels">
            <label id="number-label" htmlFor="currentUnitPrice">Harga saat ini (dalam unit): </label>
          </div>
          <div className="rightTab" key={"currentUnitPrice"}>
            <input type="text" name="currentUnitPrice" id="currentUnitPrice"
                   className="input-field"
                   placeholder={"Rp"}
                   onChange={e => setCurrentUnitPriceValue(onChangeRupiah(e))}
                   value={currentUnitPriceValue} required={true} />
          </div>
        </div>
        <div className="rowTab">
          <div className="labels">
            <label id="number-label" htmlFor="monthlyInvestTarget">Target investasi bulanan: </label>
          </div>
          <div className="rightTab" key={"monthlyInvestTarget"}>
            <input type="text" name="monthlyInvestTarget" id="monthlyInvestTarget"
                   className="input-field"
                   placeholder={"Rp"}
                   onChange={e => setMonthlyInvestTargetValue(onChangeRupiah(e))}
                   value={monthlyInvestTargetValue} required={true} />
          </div>
        </div>
        <div>
          <div style={{
