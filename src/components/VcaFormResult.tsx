...
export const getVcaResult = (stock: VcaCalculator) => {
  ...
  return (
    <>
      ...
      <p>
        {lotShouldInvest >= 0
          ? `maka bulan ini saya harus investasi sebanyak: ${lotShouldInvest} lot (${lotShouldInvest * 100} unit) senilai ${rp(
              lotShouldInvest * stock?.currentLotPrice
            )}`
          : `maka bulan ini saatnya taking profit dulu ${Math.abs(lotShouldInvest)} lot (${Math.abs(lotShouldInvest) * 100} unit) senilai ${rp(
              Math.abs(lotShouldInvest * stock?.currentLotPrice)
            )}🤑`}
      </p>
      <p>{`lalu jumlah stock yang akan saya miliki menjadi ${(stock?.totalLot + lotShouldInvest).toLocaleString("id-ID")} lot
